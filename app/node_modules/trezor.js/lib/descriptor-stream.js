...
var DescriptorStream = function (_EventEmitter) {
    ...
    function DescriptorStream(transport) {
        ...
        _this.debugAcquiredEvent = new _flowEvents.Event1('debugAcquired', _this);
        _this.debugReleasedEvent = new _flowEvents.Event1('debugReleased', _this);
        _this.debugChangedSessionsEvent = new _flowEvents.Event1('debugChangedSessions', _this);
    ...
    _createClass(DescriptorStream, [{
        key: 'setHard',
        value: function setHard(path, session, debug) {
            if (this.previous != null) {
                var copy = this.previous.map(function (d) {
                    if (d.path === path) {
                        if (debug) {
                            var debugSession = session;
                            return _extends({}, d, { debugSession: debugSession });
                        }
        ...
        value: function _diff(previousN, descriptors) {
            ...
            var debugChangedSessions = descriptors.filter(function (d) {
                var previousDescriptor = previous.find(function (x) {
                    return x.path === d.path;
                });
                if (previousDescriptor !== undefined) {
            ...
            var debugAcquired = debugChangedSessions.filter(function (descriptor) {
                return descriptor.debugSession != null;
            });
            var debugReleased = debugChangedSessions.filter(function (descriptor) {
                return descriptor.debugSession == null;
            });

            var didUpdate = connected.length + disconnected.length + changedSessions.length + debugChangedSessions.length > 0;
            ...
            return {
                ...
                debugChangedSessions: debugChangedSessions,
        ...
        value: function _reportChanges() {
            ...
            return (0, _connectionLock.lock)(function () {
                ...
                if (diff.didUpdate && _this3.listening) {
                    ...
                    diff.debugAcquired.forEach(function (d) {
                        _this3.debugAcquiredEvent.emit(d);
                    });
                    diff.debugReleased.forEach(function (d) {
                        _this3.debugReleasedEvent.emit(d);
                    });
                    diff.debugChangedSessions.forEach(function (d) {
                        _this3.debugChangedSessionsEvent.emit(d);
                    });
